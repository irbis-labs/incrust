...
const ESCAPE_TAG_LIST: [(char, &str); 18] = [
    ('"', "&quot;"),
    ('&', "&amp;"),
    ('\'', "&apos;"),
    ('<', "&lt;"),
...
pub struct HtmlEscapeStrict<T>(pub T);

impl<T> fmt::Display for HtmlEscapeStrict<T>
where
    T: fmt::Display,
{
...
mod tests {
    ...
    fn test_by_list() {
        ...
        for ch in '\x20'..'\x7f' {
            ...
            let result: PascalString<7> = HtmlEscapeStrict(sample).to_fmt();
    ...
    fn html_escape_bad_attr() {
        let source = r#""x" + "y" = "xy""#;
        let sample = r"&quot;x&quot; &#43; &quot;y&quot; &#61; &quot;xy&quot;";
        assert_eq!(sample, HtmlEscapeStrict(source).to_string());
    }
}
